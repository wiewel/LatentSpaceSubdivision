...
def get_conv_shape(tensor, data_format='NHWC'):
    ...
    if data_format == 'NCHW':
        assert False, "Fix this function for 3d"
...
def denorm_img_numpy(norm):
    c = norm.shape[-1]
    in_shape = norm.shape
    if c == 2:
        norm = np.concatenate([norm, np.zeros([in_shape[0], in_shape[1], in_shape[2], 1])], axis=-1)
    elif c > 3:
        norm = norm[:,:,:,:3]
    return np.asarray( np.clip((norm + 1)*127.5, 0, 255) , dtype="uint8" )
...
def jacobian3(x):
    ...
    dvdx = x[:,:,:,1:,1] - x[:,:,:,:-1,1]
    dwdx = x[:,:,:,1:,2] - x[:,:,:,:-1,2]
    dudy = x[:,:,:-1,:,0] - x[:,:,1:,:,0] # y flipped
    dvdy = x[:,:,:-1,:,1] - x[:,:,1:,:,1] # y flipped
    dwdy = x[:,:,:-1,:,2] - x[:,:,1:,:,2] # y flipped
    dudz = x[:,1:,:,:,0] - x[:,:-1,:,:,0]
    dvdz = x[:,1:,:,:,1] - x[:,:-1,:,:,1]
    dwdz = x[:,1:,:,:,2] - x[:,:-1,:,:,2]
...
def jacobian(x, data_format='NHWC'):
    print(x.shape)
    if data_format == 'NCHW':
        x = nchw_to_nhwc(x)
...
def vort(x, data_format='NHWC'):
    ...
    vort = tf.expand_dims(dvdx - dudy, axis=-1)
    if data_format == 'NCHW':
        vort = nhwc_to_nchw(vort)
    return vort
...
def isolate_streamfunction(x, data_format='NHWC'):
    return tf.expand_dims(x[...,0], axis=-1)

def pad_zero_2d(x, data_format='NHWC'):
    if data_format == 'NCHW':
        x = nchw_to_nhwc(x)
    # pad top and right by one
    x = tf.pad(x, [[0,0], [0, 1], [0, 1], [0,0]], "CONSTANT", constant_values=0)
    if data_format == 'NCHW':
        x = nhwc_to_nchw(x)
    return x

def curl_padded(x, data_format='NHWC'):
    if data_format == 'NCHW':
        x = nchw_to_nhwc(x)

    u = x[:,:-1,:,0] - x[:,1:,:,0] # ds/dy, horizontally flipped
    v = x[:,:,:-1,0] - x[:,:,1:,0] # -ds/dx,
    c = tf.stack([u[:,:,:-1],v[:,:-1,:]], axis=-1)
...
def plane_view_np(x, xy_plane=True, project=True):
    ...
    if xy_plane:
        ...
        if project:
            x = np.mean(x, axis=3).transpose([0,2,1,3])
        else:
            x = x[:,:,:,c_id[1],:].transpose([0,2,1,3])
