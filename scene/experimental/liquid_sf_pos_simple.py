...
parser.add_argument("--log_dir", type=str, default='data/liquid_sf{}-{}-{}_pos_simple{}_f{}')
...
def main():
	...
	m.vel_air 		= m.s.create(MACGrid,		name="vel_air")
	...
	for i in trange(args.num_scenes, desc='scenes'):
		...
		m.vel_air.copyFrom(m.vel)
		extrapolateMACSimple( flags=m.flags, vel=m.vel_air, distance=1 )

		for t in range(args.num_frames):
			m.vel.copyFrom(m.vel_air)
			...
			m.vel_air.copyFrom(m.vel)
			extrapolateMACSimple( flags=m.flags, vel=m.vel_air, distance=1 )
			setWallBcs(flags=m.flags, vel=m.vel_air)
			solvePressure(flags=m.flags_air, vel=m.vel_air, pressure=m.pressure_air, cgMaxIterFac=0.5, cgAccuracy=args.accuracy)
