...
parser.add_argument("--log_dir", type=str, default='data/liquid_sf{}-{}-{}_pos_simple_singlevel{}_f{}')
...
def main():
	print("This is a test scene for liquid velocity stream functions.")
	...
	m.pressure_air 	= m.s.create(RealGrid, 		name="pressure_air")
	m.phi_air 		= m.s.create(LevelsetGrid, 	name="phi_air")
	m.flags_air 	= m.s.create(FlagGrid, 		name="flags_air")
	...
	for i in trange(args.num_scenes, desc='scenes'):
		...
		m.flags_air.initDomain(boundaryWidth=args.bWidth)
		...
		if args.open_bound:
			bound_str = 'xXyYzZ' if is_3d else 'xXyY'
			setOpenBound(m.flags, args.bWidth, bound_str, FlagOutflow|FlagEmpty)

		# Solve for air movement
		m.phi_air.copyFrom(m.phi)
		m.phi_air.multConst(-1.0) # negate levelset to transform from liquid to air
		m.phi_air.setBound(args.bWidth, 1.) # enforce outside values at border
		if args.open_bound:
			resetOutflow(flags=m.flags, phi=m.phi_air) # open boundaries

		m.vel.clear()
		m.pressure.clear()

		m.pressure_air.clear()
		...
		for t in range(args.num_frames):
			...
			# Solve for air movement
			m.phi_air.copyFrom(m.phi)
			m.phi_air.multConst(-1.0)

			m.phi_air.setBound(args.bWidth, 1.) # enforce outside values at border
			if args.open_bound:
				resetOutflow(flags=m.flags, phi=m.phi_air) # open boundaries
			m.flags_air.clear()
			m.flags_air.updateFromLevelset(m.phi_air)
			m.flags_air.replaceFlag(CellType_TypeEmpty, CellType_TypeObstacle)
			setOpenBound(m.flags_air, args.bWidth, 'xXyYzZ' if is_3d else 'xXyY', FlagOutflow|FlagEmpty)

			extrapolateMACSimple( flags=m.flags, vel=m.vel, distance=1 )
			setWallBcs(flags=m.flags, vel=m.vel)
			solvePressure(flags=m.flags_air, vel=m.vel, pressure=m.pressure_air, cgMaxIterFac=0.5, cgAccuracy=args.accuracy)
